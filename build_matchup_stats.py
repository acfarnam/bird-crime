...
SEASONS = {
    ...
    # 2026 intentionally excluded — no matchups exist yet this season
}
...
def main():
    ...
    for season in SEASONS:
        season_file = os.path.join(DATA_DIR, "seasons", f"{season}.json")
        if os.path.exists(season_file):
            sd = load_local(season_file)
            final_standings_by_season[season] = {
                row["owner_id"]: {"wins": row["wins"], "losses": row["losses"]}
